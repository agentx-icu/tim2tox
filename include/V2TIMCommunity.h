...
enum V2TIMTopicInfoModifyFlag {
    ...
    /// 话题类型（从 8.5 版本开始支持）
    V2TIM_COMMUNITY_MODIFY_FLAG_TOPIC_TYPE = 0x1 << 24,
    /// 申请加入话题选项(私密话题有效)（从 8.4 版本开始支持）
    V2TIM_COMMUNITY_MODIFY_FLAG_TOPIC_ADD_OPTION = 0x1 << 25,
    /// 话题邀请选项(私密话题有效)（从 8.4 版本开始支持）
    V2TIM_COMMUNITY_MODIFY_FLAG_TOPIC_APPROVE_OPTION = 0x1 << 26,
};
...
struct TIM_API V2TIMTopicInfo : V2TIMBaseObject {
    ...
    /// 如果您同时修改多个字段，多个枚举值按位或 | 组合，例如，同时修改话题名称和头像
