...
void main() {
  group('ToxAV Conference Audio Send Tests (Virtual)', () {
    ...
    test('AV conference with audio sending', () async {
      ...
      for (var attempt = 0; !bothArrived && attempt < 4; attempt++) {
        ...
        try {
          await waitUntilWithAvVirtualPump(
            ...
            description:
                'Both received invites (attempt ${attempt + 1})',
            advanceMs: 200,
            iterationsPerInstance: 4,
            wallSleep: const Duration(milliseconds: 50),
          );
      ...
      final bobJoinResult = await bob.runWithInstanceAsync(
      ...
      final charlieJoinResult = await charlie.runWithInstanceAsync(
          () async => TIMManager.instance.joinGroup(
      ...
      await pumpTestTickAv(scenario,
          advanceMs: 5000, iterationsPerInstance: 2);
